...
const CheckoutPage = () => {
  ...
  const handleWhatsAppCheckout = () => {
    const baseUrl = "http://212.47.79.90:3000";
    const items = products.map((p: any) => 
      `• ${p.title} x${p.amount} — KES ${p.price * p.amount}\n  ${baseUrl}/product/${p.slug || ""}`
    ).join("\n");
    const subtotal = products.reduce((sum: number, p: any) => sum + p.price * p.amount, 0);
    const message = `Hello! I would like to order:\n\n${items}\n\nSubtotal: KES ${subtotal}\nShipping: KES 5\nTotal: KES ${Math.round(subtotal + subtotal / 5 + 5)}\n\nName: ${checkoutForm.name} ${checkoutForm.lastname}\nPhone: ${checkoutForm.phone}\nAddress: ${checkoutForm.adress}, ${checkoutForm.city}\n\nPlease confirm my order.`;
    const phone = "254732351627";
            ...
            <dl className="hidden space-y-6 border-t border-gray-200 pt-6 text-sm font-medium text-gray-900 lg:block">
              ...
              <div className="flex items-center justify-between border-t border-gray-200 pt-6">
                ...
                <dd className="text-base">
                  KES {total === 0 ? 0 : Math.round(total + total / 5 + 5)}
                </dd>
              </div>
            </dl>
          </div>
...
<div className="mt-10 border-t border-gray-200 pt-6 ml-0">
              ...
              <button
                ...
                className="mt-4 w-full rounded-md border border-transparent bg-green-500 px-20 py-2 text-lg font-medium text-white shadow-sm hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-600"
              >
                🟢 Order via WhatsApp
              </button>
            </div>
